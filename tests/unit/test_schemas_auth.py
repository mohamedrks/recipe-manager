...
def test_valid_user_register_request() -> None:
    user = UserRegisterRequest(email="test@example.com", password="SecurePass123")
...
def test_password_too_long_rejected() -> None:
    with pytest.raises(ValidationError):
        UserRegisterRequest(email="test@example.com", password="x" * 101)
